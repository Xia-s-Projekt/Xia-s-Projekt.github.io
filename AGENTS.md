...
- **Navigation is viewport-split.** Desktop (>900px) keeps a fixed top nav (`nav`, height `--nav-h: 60px`, `z-index: 100`, blurred glass): `.nav-logo` (gradient text, click → home), `.nav-controls` with Donate (`.btn-dl primary`) and Theme (`.btn-dl secondary`, `#theme-toggle`). Mobile / tablet portrait (≤900px) hides `nav` entirely and uses the **floating pill dock** (`nav.mobile-dock`, base `display: none`, shown via `display: flex` inside the media query; full-width fixed containing block with `margin: 0 auto` + `width: max-content` + `max-width: calc(100vw - 24px)` centering, `border-radius: 100px`, `z-index: 100` below modals, `env(safe-area-inset-bottom)` offset): gradient brand mark (`.dock-brand`, clickable → home, hidden ≤900px since the hero shows the brand), plus pill buttons: Back (ghost, only visible inside `?os=` detail via `.mobile-dock.is-detail`), Donate (.dock-primary accent pill), and Theme (ghost, `#dock-theme`). Desktop gets `padding-top: calc(var(--nav-h) + …)` so content clears the nav; both pages get ~120px bottom padding so content clears the dock. On ≤900px the dock tightens (smaller padding/buttons), `.back-btn` on the detail page is hidden (dock Back replaces it), and the catalog defaults to `list` view (`.cards-grid.view-list` = single column, tighter gap; choice persisted in `localStorage('view-mode')`).
